...
You can create objects using the **assignment operator**, `<-`. This evaluates the expression on its right hand side and assigns the result to the object name on its left.^[Technically you can use either `<-` or `=` for variable assignment. Both do the same thing when used for assignment, though in general there is a subtle difference between the two.]  
...
You should experiment with R Markdown and get used to its syntax. Here is a R Markdown cheatsheet `r xfun::embed_file('./downloads/rmd-cheatsheet.pdf')` to get started.  
