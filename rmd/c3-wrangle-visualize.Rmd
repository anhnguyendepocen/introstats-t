...
```{r}
# extract column 'price_USD' and assign to new object 'prices'
prices <- nycbnb$price_USD
```
...
```{r}
# select columns 'price_USD' and 'borough' and assign to new object 'prices_and_borough'
prices_and_boroughs <- select(nycbnb, price_USD, borough)
```
...
`r margin_note("Note since these operations are assigned to an existing object, 'nycbnb', you should find they overwrite the contents of the original data frame. If you want to preserve the original data you should assign your changes to a new data object.")`
...
```{r}
# add new column 'revenue' where revenue = price_USD*availability_365 
nycbnb <- mutate(nycbnb, revenue = price_USD*availability_365)
```
...
nycbnb_manh <- nycbnb %>%
  ...
  select(-borough) %>%
  mutate(price_GBP = price_USD*0.8)
```
...
```{r, warning=FALSE}
# prices_by_borough <- prices_and_boroughs
#   group_by(borough) %>%                          # group by 'borough'
#   summarize(avg_price_USD = mean(price_USD))     # aggregate price by mean
```
...
The following code plots a histogram of the variable `price_USD`:
...
```{r}
ggplot(data = nycbnb, mapping = aes(x = price_USD)) + 
...
Below is a scatterplot of price on latitude on longitude:
...
To distinguish between boroughs, you can add another argument to the aesthetic mapping, `color = borough`. This will use different colors for data points corresponding to each borough:^[For obvious reasons, the color mapping requires a categorical variable. A different color will be used for each category.]
...
Below is a box and whisker plot of price (integer) on borough (categorical): 

```{r}
ggplot(data = nycbnb, mapping = aes(x = borough, y = price_USD)) +
  geom_boxplot() +
  xlab('borough') +
  ylab('price in usd') +
  theme_light()
