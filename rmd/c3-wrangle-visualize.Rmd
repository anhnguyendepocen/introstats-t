...
`r margin_note("Note, many functions in R accept both US and UK spellings (where there is a difference)--e.g. both $\\texttt{summarise()}$ and $\\texttt{summarize()}$ will work.")`
...
`r margin_note("For obvious reasons, the color mapping requires a categorical variable. A different color (or colour) will be used for each category.")`
